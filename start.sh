...
setup_vm_image() {
    ...
    # Tạo password hash cho cloud-init
    local pass_hash
    if command -v mkpasswd &>/dev/null; then
        # Sử dụng mkpasswd nếu có
        pass_hash=$(mkpasswd -m sha-512 "$PASSWORD")
    elif command -v openssl &>/dev/null; then
        # Sử dụng openssl nếu mkpasswd không có
        pass_hash=$(openssl passwd -6 "$PASSWORD")
    else
        # Tạo hash đơn giản nếu không có công cụ nào
        pass_hash=$(echo -n "$PASSWORD" | sha512sum | cut -d' ' -f1)
        pass_hash="\$6\$${pass_hash:0:8}\$${pass_hash}"
    fi
